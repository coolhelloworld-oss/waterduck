...
form](https://github.com/WaterduckBrowser/waterduck/issues/new?template=bug_report.yml), which helps to ensure that, for us to reproduce and investigate the problem, your bug report includes the information needed — including a reduced test case.
...
3. Open/load the `REDUCTION.html` file in Waterduck, and verify that the same problem occurs with it as occurs with the original website/page you copied it from.

4. **Script-related problems:** Especially if you believe the problem is related to any JavaScript that the document is executing, then temporarily disable scripting by unchecking **Enable Scripting** option in the **Debug** menu in Waterduck, and then reload the `REDUCTION.html` file in Waterduck.
...
5. **CSS-related problems**: Especially if you believe that the problem is related to any CSS stylesheets the document is using, then:
   ...
   2. Start removing CSS rules from any and all `style` elements in the document, and reload the `REDUCTION.html` file in Waterduck.
...
7. Reload the `REDUCTION.html` file in Waterduck, and verify whether the problem is still happening.
...
When investigating a bug, it can be helpful to use the `--enable-idl-tracing` command-line flag when running Waterduck. This will output detailed information about the calls being made to the browser's internal interfaces, which can help pinpoint where a problem is occurring.
